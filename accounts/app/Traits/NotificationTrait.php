    ...
    public function sendNotiOnesignalMutile($_data = [],$type)
    {
        ...
        foreach ($_data as $j => $data) {
            ...
            if (!empty($notification)) {
                ...
                if ($type == Config::get('constant')['noti_remind']){
                    $transaction = Transaction::find($data['object_id']);
                    $transaction->noti = 1;
                    $transaction->save();
                }
                if ($type == Config::get('constant')['noti_remind_one']){
                    $transaction = Transaction::find($data['object_id']);
                    $transaction->noti_one = 1;
                    $transaction->save();
                }
                if ($type == Config::get('constant')['noti_reset_use_point']){
                    $client = Clients::find($data['object_id']);
                    $client->date_reset_point = date('Y-m-d');
                    $client->save();
                    changePoint($client->id,'reset');
                }
                $notification->created_at_new = _dt_new($notification->created_at);
                ConnectPusher($notification, $data['arr_object_id'], 'notification');
